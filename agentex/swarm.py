...
class Swarm:
    ...
    def notify_task_completed(self, agent, result):
    ...
    async def async_notify_task_completed(self, agent, result):
        """Asynchronously notify that an async task is completed."""
        self.logger.dprint(f"[ASYNC] Agent '{agent.name}' completed async task. Result: {result}", level="info")
        self.agent_queue.append(agent)
